...
function isCreditCardNumber(ccn) {
  const cardArray = ccn.toString().split('').map((elem) => Number(elem)).reverse();
  let sum = 0;
  for (let i = 0; i < cardArray.length; i += 1) {
    if (i % 2 === 1) {
      let tmp = 2 * cardArray[i];
      if (tmp > 9) {
        tmp -= 9;
      }
      sum += tmp;
...
function getDigitalRoot(num) {
  const allSum = num.toString().split('').reduce((sum, elem) => sum + Number(elem), 0);
  if (allSum > 9) {
    return getDigitalRoot(allSum);
  }
  return allSum;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === '[' || str[i] === '(' || str[i] === '{' || str[i] === '<') {
      stack.push(str[i]);
    } else if (stack.pop() !== brackets[str[i]]) {
      return false;
    }
  }
  return true;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathArr[0].length; i += 1) {
    for (let j = 1; j < pathArr.length; j += 1) {
