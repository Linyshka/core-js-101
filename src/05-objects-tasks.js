...
class CssSelector {
  ...
  pseudoElement(value) {
    this.validateEIP('pseudo-element');
    this.str += `::${value}`;
    return this;
  }
  ...
  validateOrder(value) {
    ...
    if (value === 'element') {
      if (this.str.search(/[#.:[\]]/) !== -1 || this.str.search(/::/) !== -1) {
        throw error;
      }
    }
    if (value === 'id') {
...
const cssSelectorBuilder = {
  ...
  id(value) {
    if (!(this instanceof CssSelector)) {
      return new CssSelector(`#${value}`);
    }
  ...
  combine(selector1, combinator, selector2) {
    const res = `${selector1.str} ${combinator} ${selector2.str}`;

    return new CssSelector(res);
  },
